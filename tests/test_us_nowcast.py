...
def test_project_gdp_honors_fitted_convergence(gdp_level):
    """A fitted trend/persistence pair must actually steer the path."""
    slow = gdp_mod.project_gdp(gdp_level, {"fitted_qoq_pct": 2.0,
                                           "fitted_trend_qoq": 0.001,
                                           "fitted_convergence": 0.0},
                               horizon_quarters=4)
    proj = slow[slow["projected"]]["qoq_pct"].to_numpy()
    assert proj[0] == pytest.approx(2.0, abs=1e-6)
    # zero persistence -> straight to trend from the second quarter on
...
def test_estimate_convergence_needs_history(gdp_level):
    assert gdp_mod.estimate_convergence(gdp_level.tail(8)) is None


def test_convergence_falls_back_to_config(gdp_level):
    """With no fitted pair supplied, project_gdp uses the config constants."""
    out = gdp_mod.project_gdp(gdp_level, {"fitted_qoq_pct": 1.0},
                              horizon_quarters=3)
    proj = out[out["projected"]]["qoq_pct"].to_numpy()
    expected = config.GDP_TREND_QOQ * 100 + \
        (1.0 - config.GDP_TREND_QOQ * 100) * config.GDP_CONVERGENCE
    assert proj[1] == pytest.approx(expected, abs=1e-6)
